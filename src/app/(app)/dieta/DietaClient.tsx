  ...
  function renderTrocas() {
    const tbl = (t: typeof TROCAS_GUIDE.proteinas) => (
      <div style={{ overflowX: 'auto', marginBottom: 16, WebkitOverflowScrolling: 'touch' }}>
        <table style={{ width: '100%', minWidth: 420, borderCollapse: 'collapse', background: 'var(--surface)', borderRadius: 'var(--radius-lg)', overflow: 'hidden', border: '1px solid var(--border)', fontSize: 13 }}>
          <thead>
            <tr>
              {t.header.map(h => (
    ...
    // Nota mobile: tabelas de troca têm 3-4 colunas com texto que não
    // encolhe bem (nomes de alimento, medida caseira). Em vez de forçar
    // fonte minúscula ou quebrar linha no meio de "2,5 col. cheias", dou
    // minWidth na tabela e deixo o wrapper rolar horizontalmente — o
    // padrão mais previsível pra tabela de dados em tela estreita.
    return (
      <div>
