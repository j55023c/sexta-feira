  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        fontFamily: mono ? 'var(--font-dm-mono)' : 'var(--font-syne)',
        color: 'var(--text)',
      }}>
        {value}
      </div>
...
export default function CalculadoraClient({ profile, fisicoLog }: Props) {
  ...
  const [appliedMetas, setAppliedMetas] = useState(false)
  ...
  function calcFase() {
    ...
    setResultado({
      titulo, kcal, prot, carb, fat, ritmo, guia, fase,
      peso: p, metaPeso: !isNaN(m) ? m : null,
    })
    setAppliedMetas(false)
  }
  ...
  function calcProgressoPeso(r: ResultadoCalc) {
    ...
    const entries = Object.entries(fisicoLog)
      .filter(([, v]) => v?.peso)
  ...
  function handleAplicarMetas() {
    if (!resultado) return
    // Preenche campos manuais com os valores calculados
    ...
    startTransition(async () => {
      ...
      if (!result?.error) {
        setAppliedMetas(true)
        router.push('/nutricao')
      }
    })
  }

  // ── Salvar metas manuais ──────────────────────────────────────────────────

  function handleSalvarManuais() {
    const kcal  = parseInt(mKcal)
    const prot  = parseInt(mProt)
    const carbo = parseInt(mCarbo)
    const gord  = parseInt(mGord)
  ...
  // ── Calcular altura genética (Tanner) ────────────────────────────────────

  function calcAlvoGenetico() {
    const p = parseFloat(pai)
    const m = parseFloat(mae)
    if (!p || !m) return
    const alvo = sexo === 'M' ? (p + m + 13) / 2 : (p + m - 13) / 2
    setAlvoGenetico({
      val: alvo,
      range: `Intervalo esperado: ${(alvo - 10).toFixed(0)}–${(alvo + 10).toFixed(0)} cm`,
    })
  }
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: 'var(--muted)', marginBottom: 20 }}>
        Calcule suas necessidades calóricas e defina suas metas.
      </p>
      ...
      <div style={card}>
        ...
        <div style={grid3}>
          <Field label="Peso (kg)">
            <input type="number" step={0.1} placeholder="Ex: 70"
              value={peso} onChange={e => setPeso(e.target.value)} style={inputStyle} />
          </Field>
          <Field label="Altura (cm)">
            <input type="number" step={0.1} placeholder="Ex: 170"
              value={alt} onChange={e => setAlt(e.target.value)} style={inputStyle} />
          </Field>
          <Field label="Idade">
            <input type="number" placeholder="Ex: 25"
              value={idade} onChange={e => setIdade(e.target.value)} style={inputStyle} />
          </Field>
        </div>
        ...
        <button
          onClick={calcFase}
          disabled={!peso || !alt || !idade}
          style={{ ...btnP, opacity: (!peso || !alt || !idade) ? .5 : 1 }}
        >
          Calcular metas
        </button>
      </div>
          ...
          {progresso && resultado.metaPeso && (
            <div style={{ marginBottom: 14 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--muted)', marginBottom: 5 }}>
                <span>{progresso.startPeso}kg</span>
                <span>{resultado.peso}kg (atual)</span>
                <span>{resultado.metaPeso}kg</span>
              </div>
              <div style={{ height: 8, borderRadius: 4, background: 'var(--border2)', overflow: 'hidden' }}>
                <div style={{
                  height: '100%', borderRadius: 4,
                  width: `${progresso.pct.toFixed(1)}%`,
                  background: 'var(--accent)', transition: 'width .4s',
                }} />
              </div>
              <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 5, textAlign: 'center' }}>
                {progresso.diff.toFixed(1)}kg restantes
                {progresso.semanas > 0 ? ` · ~${progresso.semanas} semanas` : ''}
              </div>
            </div>
          )}

          <button
            onClick={handleAplicarMetas}
            disabled={isPending}
            style={{ ...btnP, width: '100%' }}
          >
            {isPending ? 'Aplicando...' : 'Aplicar essas metas na Nutrição'}
          </button>
          {appliedMetas && (
            <div style={{ fontSize: 12, color: 'var(--green)', marginTop: 8, textAlign: 'center' }}>
              ✓ Metas aplicadas
            </div>
          )}
        </div>
      ...
      <div style={card}>
        ...
        <div style={grid2}>
          <Field label="Kcal meta/dia">
            <input type="number" placeholder="Ex: 2000"
              value={mKcal} onChange={e => { setMKcal(e.target.value); setSavedManuais(false) }}
              style={inputStyle} />
          </Field>
          <Field label="Proteína meta (g)">
            <input type="number" placeholder="Ex: 160"
              value={mProt} onChange={e => { setMProt(e.target.value); setSavedManuais(false) }}
              style={inputStyle} />
          </Field>
        </div>

        <div style={{ ...grid2, marginBottom: 14 }}>
          <Field label="Carboidrato meta (g)">
            <input type="number" placeholder="Ex: 220"
              value={mCarbo} onChange={e => { setMCarbo(e.target.value); setSavedManuais(false) }}
              style={inputStyle} />
          </Field>
          <Field label="Gordura meta (g)">
            <input type="number" placeholder="Ex: 55"
              value={mGord} onChange={e => { setMGord(e.target.value); setSavedManuais(false) }}
              style={inputStyle} />
          </Field>
        </div>

        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={handleSalvarManuais} disabled={isPending} style={btnP}>
            Salvar metas
          </button>
          {savedManuais && (
      ...
      {/* ── Altura-alvo genética (Tanner) ── */}
      <div style={card}>
        <SectionLabel label="Perfil e altura-alvo genética" />

        <div style={grid3}>
          <Field label="Altura do pai (cm)">
            <input type="number" step={0.1} placeholder="Ex: 175"
              value={pai} onChange={e => setPai(e.target.value)} style={inputStyle} />
          </Field>
          <Field label="Altura da mãe (cm)">
            <input type="number" step={0.1} placeholder="Ex: 162"
              value={mae} onChange={e => setMae(e.target.value)} style={inputStyle} />
          </Field>
          <div style={{ display: 'flex', alignItems: 'flex-end' }}>
            <button
              onClick={calcAlvoGenetico}
              disabled={!pai || !mae}
              style={{ ...btnS, width: '100%', opacity: (!pai || !mae) ? .5 : 1 }}
            >
              Calcular
            </button>
          </div>
        ...
        {alvoGenetico && (
          <div style={{ marginTop: 10 }}>
            <div style={{
              ...
              Altura-alvo genética (fórmula de Tanner)
            </div>
            <div style={{
              fontSize: 28, fontWeight: 800,
              fontFamily: 'var(--font-dm-mono)', color: 'var(--accent)',
            }}>
              {alvoGenetico.val.toFixed(1)} cm
            </div>
